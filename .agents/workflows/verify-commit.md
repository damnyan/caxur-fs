---
name: verify-commit
description: "Run verification checks (build, lint, test), fix any warnings/errors automatically, and commit/push code if successful."
---

# Verify and Commit Workflow

This workflow enforces code quality standards, mandates zero build/lint warnings or errors, and automates the git commit and push process safely.
...
1. **Run Verification**:
   ...
   - **Strict Zero Warnings & Zero Errors Standard**:
     - Verify that all three services (`client`, `admin`, `api`) pass with zero build errors and zero lint/clippy warnings (`--max-warnings 0` for frontend, `-D warnings` for Rust).

2. **Handle & Fix Verification Failures**:
   - If verification fails or any warnings/errors are emitted, **DO NOT commit or push**.
   - Read the output and error logs immediately to isolate root causes (e.g., ESLint warnings, TypeScript build errors, Rust Clippy warnings, SQLx prepare mismatches).
   - **Fix all warnings and errors automatically**.
   - Re-run `scripts/verify-all.sh` iteratively until the entire stack compiles and verifies with **zero warnings** and **zero errors**.

3. **Generate Conventional Commit**:
   - Only after a successful verification with zero warnings and zero errors, analyze your changes using `git diff --cached` or `git status`.
